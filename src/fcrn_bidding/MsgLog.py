...
# FROM https://github.com/fbadine/util
...
def LogInit(name, filename, debuglevel=logging.INFO, log=True):
    ...
    try:
        ...
        logging.error(
            "Invalid debuglevel (%d), changing it to %d", debuglevel, logging.INFO
        )
    ...
    formatter = logging.Formatter(
        "%(asctime)s.%(msecs)d (%(process)d) (%(levelname)s) %(module)s.%(funcName)s -> %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )
