...
def plot_prob_forecasts(
    ts_entry, forecast_entry, sample_id, prediction_length, plot_length, inline=True
):
...
def evaluate(dataset_name, estimator, horizon):
    ...
    q10_, q50_, q90_, indexes_ = (
        sesies_q,
        sesies_q,
        sesies_q,
        np.empty((0, horizon * series), "datetime64[s]"),
    )
    ...
    for horizon in horizons:
        for dataset_name in datasets:
            for estimator in estimators:
                ...
                if save:
                    sub_df.to_csv(
                        r"./save/metrics_benchmarks_deepar_el.csv", index=False
                    )
